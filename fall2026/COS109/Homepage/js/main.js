...
// ---- Messenger app: reply differently to the first and later messages ----
const messageInput = document.querySelector('.messageBox');
const messageSendButton = document.querySelector('.messageSendBtn');
const messageBody = document.querySelector('#messageBody');
let sentMessageCount = 0;

function addScheduleReply() {
  // Show the QR code and schedule link only after the first sent message.
  if (messageBody.querySelector('.scheduleReply')) return;

  const reply = document.createElement('div');
  reply.className = 'scheduleReply';

  const replyText = document.createElement('p');
  replyText.textContent = 'Please scan the QR code or click Schedule.';

  const qrCode = document.createElement('img');
  qrCode.src = './img/qrcode.png';
  qrCode.alt = 'QR code for scheduling an appointment';

  const scheduleLink = document.createElement('a');
  scheduleLink.href = 'https://xinxingwu-ksu.github.io/schedule/wu.html';
  scheduleLink.target = '_blank';
  scheduleLink.rel = 'noopener noreferrer';
  scheduleLink.textContent = 'Schedule';

  reply.appendChild(replyText);
  reply.appendChild(qrCode);
  reply.appendChild(scheduleLink);
  messageBody.appendChild(reply);
  messageBody.scrollTop = messageBody.scrollHeight;
}

function addRecordedReply() {
  const reply = document.createElement('div');
  reply.className = 'receivedReply';

  const replyText = document.createElement('p');
  replyText.textContent =
    'I received your message. Your following messages will be recorded. Please explore the other panels.';
...
function sendMessengerMessage() {
  ...
  messageBubble.className = 'userTextMsg';
  ...
  if (sentMessageCount === 1) {
    window.setTimeout(addScheduleReply, 250);
  } else {
    window.setTimeout(addRecordedReply, 250);
  }
}
...
messageSendButton.addEventListener('click', sendMessengerMessage);
...
// COS 310 is displayed directly in the local Safari iframe.
